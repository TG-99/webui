 ...
 * Self-contained logic for verification, Sources scraper, and pool management.
 */
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    // Auto refresh stats and pool every 60 seconds (keeps countdown timers live)
    setInterval(() => {
      loadDashboardStats();
      loadKeyboxPool();
    }, 60 * 1000);
  ...
  function initDOM() {
    ...
    // Sources Scraper elements (Community sources)
  ...
  function setupEventListeners() {
    ...
    DOM.btnRefreshPool.addEventListener('click', () => {
      recheckKeyboxPool();
    });

    // Sources Scraper Modal Controls
    ...
    if (DOM.btnAddSourceAction && DOM.sourcesAddUrlInput) {
      DOM.btnAddSourceAction.addEventListener('click', () => {
        addSourceUrl(
    ...
    // Delegation for Pool Cards (Download, Delete, Copy)
  ...
  async function verifyKeybox() {
    ...
    if (activeMethod === 'upload') {
      xmlContent = loadedXmlContent;
    } else {
      xmlContent = DOM.xmlPaste.value.trim();
    }
  ...
  // ===== Render Results Modal (Evoker Specification) =====
  function renderKeyboxResults(res) {
    if (!DOM.resultContainer) return;

    const status = (res.status || (res.certValid && !res.revoked && res.chainValid && res.privateKeyMatch !== false 
      ? ((res.rootType === 'hardware' || res.rootType === 'rkp' || res.rootType === 'knox') && !res.isSoftbanned ? 'STRONG' : 'SOFTBAN') 
      : (res.revoked ? 'REVOKED' : 'INVALID'))).toUpperCase();
    ...
    const rootKindTag = (res.rootType === 'rkp') 
      ...
      : (res.rootType === 'hardware') 
        ? '<span class="status-pill strong" style="font-size: 0.7rem; padding: 2px 7px; margin-left: 6px;"><i class="fa-solid fa-microchip"></i> TEE</span>' 
        : (res.rootType === 'software') 
          ? '<span class="status-pill softban" style="font-size: 0.7rem; padding: 2px 7px; margin-left: 6px;"><i class="fa-solid fa-code"></i> AOSP</span>' 
          : '';
    ...
    let certsHtml = (res.certInfos || []).map(c => `
      <div style="background: rgba(255, 255, 255, 0.03); border: 1px solid var(--border-color); border-radius: var(--radius-sm); padding: 10px; margin-top: 8px; font-size: 0.78rem;">
        ...
        <div style="color: var(--text-muted); margin-top: 2px;">Validity: <span style="color: ${c.isValid ? 'var(--success)' : 'var(--danger)'};">${c.notBefore} to ${c.notAfter}</span></div>
      </div>
    `).join('');
    ...
    DOM.resultContainer.innerHTML = `
      ...
      <table class="result-meta-table">
        <tbody>
          ...
          <tr><td>Root Attestation</td><td><strong>${escapeHtml(res.rootName || res.rootType || 'Attestation Root')}</strong> ${rootKindTag}</td></tr>
          <tr><td>Earliest Expiry</td><td>${escapeHtml(res.expiresAt || 'N/A')} (${daysLeftHtml})</td></tr>
          ...
          <tr><td>Check Time (UTC)</td><td>${escapeHtml(res.checkTime || '')}</td></tr>
        </tbody>
      </table>
  ...
  function getPurgeCountdownInfo(detectedAtIso, createdAtIso) {
    ...
    try {
      ...
      const now = Date.now();
      const diffMs = purgeTime - now;

      if (diffMs <= 0) {
        return { text: 'Purging soon', urgent: true };
      }
      const hours = Math.floor(diffMs / (3600 * 1000));
      const mins = Math.floor((diffMs % (3600 * 1000)) / (60 * 1000));
      const isUrgent = hours < 1;
      return { text: `Purge in ~${hours}h ${mins}m`, urgent: isUrgent };
    } catch (e) {
  ...
  async function loadKeyboxPool() {
    ...
    try {
      ...
      if (DOM.valPoolCount) DOM.valPoolCount.textContent = keyboxes.length;

      if (!keyboxes || keyboxes.length === 0) {
      ...
      DOM.poolGridContainer.innerHTML = keyboxes.map(kb => {
        ...
        const countdownPillHtml = countdownInfo ? `
          <span class="purge-countdown-pill ${countdownInfo.urgent ? 'urgent' : ''}" title="Auto-purged 24 hours after detection (Detected: ${escapeHtml((kb.detected_revoked_at || kb.created_at || '').substring(0, 19).replace('T', ' '))})">
            <i class="fa-solid fa-clock"></i>
            ${countdownInfo.text}
          </span>
        ` : '';

        // Expiry or Days Remaining pill for active keys
        let daysPillHtml = '';
        ...
        return `
          <div class="keybox-pool-card glass-panel">
            <div class="card-top-row">
              ...
              ${countdownPillHtml}
              ${daysPillHtml}
              <span class="card-date">${escapeHtml((kb.created_at || '').substring(0, 19).replace('T', ' '))}</span>
            </div>
  ...
  function updatePoolLastCheckedDisplay(isoString) {
    ...
    const formatted = formatSourceDate(isoString);
    DOM.poolLastCheckedText.textContent = `Checked: ${formatted}`;
  }
  ...
  async function copyKeyboxXml(serialNumber, buttonEl) {
    try {
      ...
      const keyboxes = await response.json();
      const kb = keyboxes.find(k => k.serial_number === serialNumber);
  ...
  function formatSourceDate(isoString) {
    if (!isoString) return '—';
    try {
  ...
  function renderSourcesList() {
    ...
    DOM.sourcesTableBody.innerHTML = sourcesList.map((src, idx) => {
      ...
      const dateText = formatSourceDate(lastScraped);
  ...
  function appendSourcesLog(type, text) {
    ...
    item.innerHTML = `<span class="log-tag ${type}">${type.toUpperCase()}</span> <span>${escapeHtml(text)}</span>`;
